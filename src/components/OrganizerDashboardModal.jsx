...
function OrganizerDashboardModal({ isOpen, onClose }) {
    ...
    const createPickupGames = () => {
        navigate("/eventDetails"); // Redirect to the login page after password reset
      };
    ...
    return (
        <div className="fixed top-0 left-0 w-full h-full flex items-end justify-center z-50 font-Poppins text-white">
            ...
            <div className="flex flex-col gap-5 z-10 p-5">
                {/* Buttons in modal */}
                <div onClick={createPickupGames} className="flex justify-center items-center gap-[14px] bg-orange rounded-[50px] max-w-[295px] h-[70px] px-5 text-[20px] font-semibold cursor-pointer">
