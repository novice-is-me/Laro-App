...
const Matches = () => {
  ...
  const filterItems = () => {
    if (selectedFilters.length > 0) {
      let filteredItems = [];
      selectedFilters.forEach((selectedCategory) => {
        const categoryFilteredItems = ActiveMatch.filter(
          (item) => item.category === selectedCategory
        );
        const minsquarterFilteredItems = ActiveMatch.filter(
          (item) => item.minsquarter === selectedCategory
        );
        filteredItems = [
          ...filteredItems,
          ...categoryFilteredItems,
          ...minsquarterFilteredItems,
        ];
      });
  ...
  return (
    <div className="bg-[#fff6f2] w-full px-9 py-7">
        ...
        <div className="bg-[#ffeee6] p-[20px] rounded-[14px] px-20 xsm:px-0 ">
          <div className=" mt-10 flex 950px:flex-col-reverse 950px:justify-center 950px:items-center">
            <div className="flex-1 justify-between flex-col sm:flex-col xsm:text-center">
              {joinGame && (
                <div>
                  <p className="text font-Poppins mb-2 text-[#bbbbbb] 950px:text-center">
                    Result
                  </p>
                  <h1 className="text-3xl font-bold font-Poppins uppercase mb-3 950px:text-center">
                    {joinGame}
                  </h1>
                  <p className="text-[#bbbbbb] font-Poppins mb-3 950px:text-center">
                    Suggested Most Search
                  </p>
                      ...
                      <div className="w-1/2 1139:flex 1139:justify-center 1139:flex-col 1139:items-center md:w-full">
                        ...
                        <div className=" flex flex-col gap-3 mt-3 ">
                          {filters2.map((category, idx) => (
                            <button
                              onClick={() =>
                                handleFilterMinsandQuarter(category)
                              }
                              key={idx}
                              className={`w-[150px] bg-none border-orange border rounded-full py-3 font-Poppins text-[12px] text-[#666666] px-7 md:mx-auto ${
                                selectedFilters?.includes(category)
      ...
      {main === "join" && <JoinViewActiveMatches setMain={setMain} />}

      {main === "view" && <JoinViewActiveMatches setMain={setMain} />}
    </div>
  );
};
